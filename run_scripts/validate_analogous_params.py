...
ALT_GENERATE_CONFIG: Dict[
    GenerateParams.AltGenerateParamsEnum, Tuple[Callable, Iterable[Any]]
] = {
    GenerateParams.AltGenerateParamsEnum.OBS_NOISE_VAR: (np.linspace, [0.1, 1, 10]),
    GenerateParams.AltGenerateParamsEnum.LIN_TO_ANG_VEL_VAR: (
        np.geomspace,
        [14.177, 14.177, 1],
    ),
}

BASE_GENERATE_PRAMS = GenerateParams(
...
def validate_analogous_params(
    ...
    num_repeat_generate: int,
    verbose: bool = False,
) -> OMultiSweepResult:
    # Evaluate the functions and their arguments stored as keys and values, respectively
    alt_param_multiplicands_for_generation: Dict[
        GenerateParams.AltGenerateParamsEnum, np.ndarray
    ] = {}
        ...
        for _ in range(num_repeat_generate):
            ...
            sweep_results_list_per_gen_params.append(
                sweep_params(
                    ...
                    ordered_sweep_config_keys=sorted(
                        list(param_multiplicands_for_opt.keys())
                    ),
    ...
    return OMultiSweepResult(
        generate_params_list=generate_params_list, sweep_results_list=sweep_results_list
    )
...
if __name__ == "__main__":
    ...
    msr = validate_analogous_params(
        data_set_generate_from=data_set,
        alt_opt_config=ALT_OPT_CONFIG,
        alt_gen_config=ALT_GENERATE_CONFIG,
        hold_rvert_at=HOLD_RVERT_AT,
        ratio_xz_to_y_lin_vel_var=RATIO_XZ_TO_Y_LIN_VEL_VAR,
        base_generate_params=BASE_GENERATE_PRAMS,
        num_repeat_generate=NUM_REPEAT_GENERATE,
        verbose=True,
    )
    results_file_name = (
        f"analogous_parameter_sweep_{datetime.datetime.now().strftime(TIME_FORMAT)}"
    )
    ...
    fig.savefig(
        os.path.join(
            CacheManagerSingleton.SWEEP_RESULTS_PATH, results_file_name + ".png"
        ),
        dpi=500,
    )
