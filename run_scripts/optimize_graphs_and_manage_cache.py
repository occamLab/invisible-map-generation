...
if __name__ == "__main__":
    ...
    for i, map_name in enumerate(map_pattern):
        ...
        if len(matching_map) == 0:
            ...
            exit(0)
        for order, map_set in enumerate(matching_map):
            for pose in map_set.map_dct["pose_data"]:
                pose_ids.append(pose["id"])
            for key, values in map_set.map_dct.items():
                map_dictionary[key].extend(values)
            map_data.append(map_set)
    for num, pose_id in enumerate(pose_ids):
        pose_ids[num] = num
    for order, instance in enumerate(map_dictionary["pose_data"]):
        instance["id"] = pose_ids[order]
