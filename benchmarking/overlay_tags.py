...
def overlay_tags(p_path, up_path, visualize):
    ...
    for tag in p_map_data["tag_vertices"]:
        ...
        for observation in next_tag_id:
            ...
            opt_cam_trans_component = list(
                p_map_data["odometry_vertices"][associated_camera_id][
                    "translation"
                ].values()
            )
            opt_cam_rot_component = list(
                p_map_data["odometry_vertices"][associated_camera_id][
                    "rotation"
                ].values()
            )
            ...
            opt_cam_pose = B.create_simd_4x4_from_se3_quat(
                opt_cam_trans_component, opt_cam_rot_component
            )

            opt_tag_in_cam_frame = (
                B.CAMERA_POSE_FLIPPER @ np.linalg.inv(opt_cam_pose) @ opt_tag_pose
            )

            unopt_pixels = B.compute_corner_pixels(observation, up_map_data)
            opt_tag_in_unopt_frame_pixels = B.compute_corner_pixels(
                observation, up_map_data, opt_tag_in_cam_frame
            )

            if visualize:
                B.visualizing_corner_pixel_differences(
                    [unopt_pixels, opt_tag_in_unopt_frame_pixels], tag["id"], "OT"
                )
            ...
            errors.append(
                B.compute_RMS_error(unopt_pixels, opt_tag_in_unopt_frame_pixels)[0]
            )
