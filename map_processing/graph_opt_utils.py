...
def optimizer_to_map(
    vertices, optimizer: g2o.SparseOptimizer, is_sba=False
) -> OG2oOptimizer:
    ...
    for i in optimizer.vertices():
        ...
        if mode == VertexType.TAGPOINT:
            ...
            if mode == VertexType.ODOMETRY:
                ...
                pose_with_metadata = np.concatenate(
                    [pose, [i], [vertices[i].meta_data["pose_id"]]]
                )
                ...
                if is_sba:
                    ...
                    pose_with_metadata[:-1] = (
                        (SE3Quat([0, 0, -1, 0, 0, 0, 1]) * SE3Quat(pose))
                        .inverse()
                        .to_vector()
                    )
...
def optimizer_to_map_chi2(
    graph, optimizer: g2o.SparseOptimizer, is_sba=False
) -> OG2oOptimizer:
...
def get_chi2_of_edge(
    ...
    if isinstance(edge, EdgeProjectPSI2UV):
        ...
        camera_coords = (
            ...
            * edge.vertex(2).estimate().inverse()
            * edge.vertex(0).estimate()
        )
        ...
        error = (
            edge.vertex(1).estimate().inverse()
            * edge.measurement()
            * edge.vertex(0).estimate()
        )
    ...
    if math.isnan(chi2):
        # Changed Chi2 Null Returns to 0.0
        chi2 = 0.0
...
def make_processed_map_json(
    ...
    Args:
        opt_result: A dictionary containing the tag locations, odometry locations, waypoint locations, cloud anchor locations
    ...
    for curr_odom in odom_locations:
        odom_vertex_list.append(
            PGOdomVertex(
                translation=PGTranslation(
                    x=curr_odom[0], y=curr_odom[1], z=curr_odom[2]
                ),
                rotation=PGRotation(
