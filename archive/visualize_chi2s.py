...
def make_parser():
    ...
    p = argparse.ArgumentParser(
        description="Visualize the chi2 values split by edge type"
    )
    p.add_argument(
        "-m",
        action="store_true",
...
def main():
    ...
    if os.path.isfile(file_name):
        ...
        chi2s = {
            edge: [info["sum"] for info in full_chi2s[edge]]
            for edge in ("odometry", "tag", "gravity")
        }
    else:
        ...
        for run in range(total_runs):
            ...
            if args.m:
                run_chi2s = gm.subgraph_pair_optimize_and_categorize_chi2(
                    sweep[run],
                    subgraphs,
                    subgraph_1_weights=GraphManager.weights_dict[
            ...
            full_chi2s["actual_chi2s"][run] = graph_opt_utils.sum_optimizer_edges_chi2(
                optimizer
            )
            full_chi2s["total_chi2"][run] = sum(
                [run_chi2s[edge]["sum"] for edge in ("odometry", "tag", "gravity")]
            )

        with open(
            os.path.join("saved_sweeps", "chi2_visualization", file_name), "w"
        ) as file:
            json.dump({"odom_tag_ratio": sweep.tolist(), "chi2s": full_chi2s}, file)
    ...
    plt.ylabel("Chi^2")
    plot_title = (
        "Chi2 Metric by Edge Type"
        if args.m
        else "Resulting Chi^2 of Optimized Graph by Edge Type"
    )
    plt.title(plot_title)
    plt.legend()
    plt.plot(
        np.log(sweep[int(sweep.shape[0] / 2) :]),
        np.exp(-sweep[int(sweep.shape[0] / 2) :]),
        "-b",
    )
    plt.show()
    plt.plot(
