...
class Plot_3D:
    ...
    def plot_translation_3D(self):
        ...
        self.ax.set(xlabel="x", ylabel="z", zlabel="y", title="3D Plot of Map")
...
class Plot_2D:
    ...
    def plot_translation_2D(self):
        ...
        self.ax.set(xlabel="x", ylabel="z", title="2D Plot of Map")
        plt.show()
...
class Plot_2D_Animation:
    ...
    def animate(self):
        ...
        fig, self.ax = plt.subplots()
        self.ax.set(
            xlabel="x", ylabel="z", title="Test Translation Mapping Data 2D Plot"
        )
        ...
        ani = animation.FuncAnimation(
            fig,
            lambda x1, x2, x3, x4: self.update(x1, x2, x3, x4),
            frames=len(x),
            fargs=[self.x, self.y, self.poseID],
            interval=poseID_interval,
            blit=True,
...
class Plot_Chi2_Animation:
    ...
    def animate(self):
        # set up graphing
        fig, self.ax = plt.subplots()
        self.ax.set(xlabel="poseID", ylabel="adjChi2", title="adjChi2 Plot")
        ...
        ani = animation.FuncAnimation(
            fig,
            lambda x1, x2, x3: self.update(x1, x2, x3),
            frames=len(x),
